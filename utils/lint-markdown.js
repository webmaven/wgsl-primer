...
const markdownFiles = args.length > 0
  ? args.filter(file => file.endsWith('.md')).map(file => path.resolve(file)).filter(file => file.startsWith(DOCS_DIR))
  : walk(DOCS_DIR);
...
markdownFiles.forEach(file => {
    ...
    for (let i = closingFrontmatterLineIndex + 1; i < lines.length; i++) {
      ...
      if (!inCodeBlock && line.trim() !== '') {
        if (line.trim().startsWith('#')) {
          console.error(`\x1b[31mError:\x1b[0m Redundant leading heading found in ${file}:${i + 1}`);
          console.error(`  Line ${i + 1}: ${line.trim()}`);
          console.error(`  Please remove the leading heading. The page title is automatically generated from the frontmatter title.\n`);
          hasErrors = true;
        }
        break;
      }
  ...
  lines.forEach((line, index) => {
    ...
    if (LEGACY_ADMONITION_REGEX.test(line)) {
      console.error(`\x1b[31mError:\x1b[0m Legacy blockquote admonition syntax found in ${file}:${index + 1}`);
      console.error(`  Line ${index + 1}: ${line.trim()}`);
      console.error(`  Please convert to standard PyMdown syntax (e.g., '!!! note "Title"') with 4-space indentation.\n`);
      hasErrors = true;
    }
    ...
    if (!insideCodeBlock && match) {
      ...
      if (index + 1 < lines.length) {
        ...
        if (nextLine.trim() !== '') {
          console.error(`\x1b[31mError:\x1b[0m Missing blank line following fenced code block closing backticks in ${file}:${index + 1}`);
          console.error(`  Closing Fence (Line ${index + 1}): ${line.trim()}`);
          console.error(`  Immediate Next Line (Line ${index + 2}): ${nextLine.trim()}`);
          console.error(`  Please insert a blank line after the closing backticks to satisfy parser nesting/spacing requirements.\n`);
          hasErrors = true;
        }
      }
    }
    ...
    if (!insideCodeBlock) {
      ...
      if (hasFrontmatter && !isHome && index > closingFrontmatterLineIndex) {
        if (/^\s*#\s+/.test(line)) {
          ...
          console.error(`  Line ${index + 1}: ${line.trim()}`);
          console.error(`  Secondary H1 headings are prohibited in lesson pages. Use H2 (##) or lower for sub-sections.\n`);
          hasErrors = true;
        }
      }
      ...
      if (/^\s*#\s+/.test(line)) {
        ...
        if (nextNonEmptyIndex !== -1) {
          ...
          if (/^\s*##\s+/.test(nextLine)) {
            ...
            console.error(`  Immediate H2 Heading (Line ${nextNonEmptyIndex + 1}): ${nextLine.trim()}`);
            console.error(`  Having an H1 heading immediately followed by an H2 subheading without intervening prose is prohibited. Please remove the redundant heading or add introductory text.\n`);
            hasErrors = true;
          }
        }
      }
      ...
      PEDAGOGICAL_PATTERNS.forEach(pattern => {
        if (pattern.regex.test(line)) {
          console.error(`\x1b[31mError:\x1b[0m Conversational/pedagogical phrase "${pattern.phrase}" found in ${file}:${index + 1}`);
          console.error(`  Line ${index + 1}: ${line.trim()}`);
          console.error(`  Please rephrase to maintain an objective, technical, reference-style tone as required by STYLE_GUIDE.md.\n`);
          hasErrors = true;
        }
      });
    }
